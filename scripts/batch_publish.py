...
def parse_chapter_file(filepath: Path) -> tuple[str, str]:
    ...
    File structure (generated by inkos):
      # 第N章 标题          ← inkos wrapper (title line)
                            ← blank line
      <!-- think -->        ← AI notes (model output, varies)
      ...
      ## 标题               ← model output (content starts here)
      正文...
      ---
      正文...

    Rule: everything from the first ``## `` line onward is the story content.
    The title is extracted from the ``# `` line (first line).
    """
    raw = filepath.read_text(encoding="utf-8")
    lines = raw.split("\n")

    # Extract title from the ## heading line (model output)
    title = filepath.stem
    for line in lines:
        if line.startswith("## "):
            title = line[3:].strip()
    ...
    # Fallback: use # heading if no ## found
    ...
    # Find the first ## line — content starts there
    content_start = 0
    ...
    # Strip the ## heading line from content (title already extracted)
    content = re.sub(r"^##\s+.+\n*", "", content)
